...
class Node:
    ...
    # this is the actual problem it's asking for -- the smallest directory of at least 389918
    def solve(self):
        solution = None
        if self.type == DIR and self.size >= 389918:
            solution = self
        for child in self.children:
            thisun = child.solve()
            if thisun != None and thisun.size < solution.size:
                solution = thisun
        return solution
